# 0.10.25 — native client heap diagnostic

[Download Wurm-Server.apk](https://github.com/Russianranger/wurm-android/releases/download/v0.10.25-native-heap/Wurm-Server.apk).
...
- Package the pinned NDK's ARM64 ASan and shared C++ runtimes. Preload ASan first
...
1. Keep the working older apps and exports. Install the separate `heaptrace`
   preview and import the same complete client and server runtime ZIPs.
   Stop the older server before starting this one.
2. Use an unused player name such as **Thortrace**. Client login identity still
...
The host test executes the same native startup check, then verifies that an
...
requires ASan instrumentation in each client native library, the startup marker,
