# 0.9.0: native JVM graphics test on the AYN Thor
...
1. Keep the working **0.6.0 server**, its world, and your **0.8.0 client** installed.
   Download `Wurm-Server.apk` from
   [v0.9.0-graphics-probe](https://github.com/Russianranger/wurm-android/releases/tag/v0.9.0-graphics-probe)
   on the Thor and install it. It is a separate `.graphicsprobe` package. The first
   screen identifies **0.9.0** so you can distinguish the apps.
...
3. In 0.9.0, open **Client tab → JVM Graphics Test → Run Graphics Test**. Allow
...
6. In **Client tab → Export Client Report**, export **`wurm-client-report.txt`**
   ...
   If Android closes the app, reopen 0.9.0 and export its retained report.
...
presentation. **Physical acceptance remains pending.** This is an EGL pbuffer,
...
Local verification: **51 Python tests** and **17 managed Kotlin/JUnit tests**;
