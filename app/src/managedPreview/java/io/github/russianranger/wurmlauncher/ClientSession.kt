...
object ClientSession {
    ...
    fun report(context: Context): String {
        ...
        return "Wurm client milestone 0.10.6\nAndroid ${android.os.Build.VERSION.RELEASE}; API ${android.os.Build.VERSION.SDK_INT}\n" +
            "Status: ${state.phase} — ${state.detail}\nDefault target: 127.0.0.1:3724\n" +
            "Gate status: 0.10.0 window/controller diagnostic passed on Thor; 0.10.1 profile/resources passed on Thor; 0.10.2 font rasterization passed on Thor; 0.10.3 created the Wurm window on Thor; 0.10.4 passed measured FBO support on Thor; 0.10.5 passed buffer cleanup and rendered the Wurm splash on Thor; blur shader/uniform query corrections await Thor test. Audio currently falls back to silent mode. Full Wurm rendering, server ticket acceptance and login are not qualified.\n\n" +
