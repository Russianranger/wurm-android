...
class RuntimeObservationLog(private val file: File) {
    @Synchronized fun observe(line: String) {
        if (!line.startsWith("[runtime-memory] ") && !line.startsWith("[graphics-error] ") && !line.startsWith("[graphics-depth] ") &&
            ...
            !line.startsWith("[client-text-buffers] ") && !line.startsWith("[client-memory-test] ") && !line.startsWith("[client-ui] FPS_APPLIED ") &&
            ...
            !line.startsWith("[diagnostics] ") && !isGcEvidence(line)) return
    ...
    private fun isGcEvidence(line: String): Boolean = line.startsWith("[20") &&
        ((line.contains("][gc") && line.contains(" GC(")) ||
            (line.contains("][safepoint") && line.contains("Safepoint")))
}
