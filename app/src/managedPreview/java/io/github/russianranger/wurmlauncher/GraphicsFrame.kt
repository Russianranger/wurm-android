...
data class GraphicsFrame(val width: Int, val height: Int, val sequence: Int, val argb: IntArray, val pointer: Pointer? = null,
    ...
    fun decodedArgb(destination: IntArray? = null): IntArray {
        val raw = rawRgba ?: return argb
        val pixels = destination ?: IntArray(width * height)
        require(pixels.size >= width * height)
        for (i in 0 until width * height) {
            val k = ((height - 1 - i / width) * width + i % width) * 4
            pixels[i] = (0xff shl 24) or ((raw[k].toInt() and 255) shl 16) or
    ...
    companion object {
        ...
        fun readNewer(file: File, afterSequence: Int): GraphicsFrame? = readNewer(file, afterSequence, null)
        internal fun readNewer(file: File, afterSequence: Int, storage: FrameBuffers.Storage?): GraphicsFrame? = DataInputStream(file.inputStream().buffered()).use { input ->
            ...
            val bytes = storage?.bytes(size * 4) ?: ByteArray(size * 4)
            input.readFully(bytes, 0, size * 4)
            val pixels = if (version == 3) EMPTY_PIXELS else (storage?.ints(size) ?: IntArray(size)).also {
                java.nio.ByteBuffer.wrap(bytes, 0, size * 4).asIntBuffer().get(it, 0, size)
            }
